...
- Lifted snackbars above the floating bar (Undo, dialer-error toasts) by padding the `SnackbarHost` by the bar's shell height on bottom-nav routes, so they no longer overlap the bar.
