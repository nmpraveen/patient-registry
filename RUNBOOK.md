...
## PhoneBook contact permissions

Admin > User Management > Roles > Manage PhoneBook contacts grants contact creation, editing, deactivation/reactivation and inactive inspection without settings administration. Migration patients.0045 grants existing Reception and settings-manager roles only and preserves all other customized fields. Admin/Reception defaults enable the permission; normal initialization does not reset subsequent revocation. Custom roles must be granted explicitly. Announcement management and unrestricted reminders still use manage_settings.

Verify with `python manage.py test staff_directory staff_announcements staff_reminders patients.test_phonebook_migration`, migration drift checks and a fresh PostgreSQL migration. Test both allowed Reception writes and denied direct web/API writes after revocation; verify Settings and announcement management remain denied and reminder owner/assignee scope is unchanged. Use the standard backup/independent-restore/plan/apply production gates below for deployment. Acceptance artifacts are retained in `output/phonebook-permission-20260916/`; browser screenshots are in `output/playwright/phonebook-permission-20260916/`.
...
MT- followed by digits is reserved for permanent identifiers, with case/whitespace normalization. Patient/Case database constraints reject direct SQL bypass; forms/model saves and every supported bundle version reject reserved UHIDs. An existing Patient or Case with such a legacy hospital ID stops 0043 before issuance/linkage for an explicit reviewed reconciliation decision. Preserve the original data; do not silently rename/drop identifiers to force migration. The remaining hospital/TMP formats retain their existing behavior.
...
For the September 20 remediation, read [the rollout and disposition ledger](docs/review-remediation-20260920.md). Pause `medtrack-security-evidence-export.timer` while changing source to a release requiring `patients.0046`; after migration, verify the installed runner and fresh exports as below before restoring its previous active state. The acknowledgement/progress tables are recoverable delivery state, not a replacement for the immutable AuditEvent table. First export reconciles exact retained UUIDs and may safely replay older events; never backfill acknowledgements from an ID watermark. Deployment `apply`/`rollback` and destructive restore modes share `/run/medtrack-operations.lock` (override only to another trusted absolute path with `MEDTRACK_OPERATION_LOCK`).

Before resuming that timer, run `./scripts/verify-security-evidence-runner.sh` from the reviewed checkout. It reads the effective systemd `ExecStart` and requires the executable to be byte-identical to the reviewed exporter, including when a drop-in selects a root-owned `/usr/local/sbin` copy. A checkout update or successful service result does not prove that the installed runner changed. If the check fails, keep the timer paused, wait for the oneshot to finish, archive the old runner privately, and refresh only the intended executable from the reviewed source; preserve existing unit hardening and configuration. Rerun the check before starting the service. Inspect other overridden backup runner paths for the same stale-copy risk; do not overwrite configured secrets or rerun unrelated installers.

Accept the fresh export only when its segment contains `audit_selection=unacknowledged-event-uuid-v1`, the acknowledgement table is populated, and its progress sequence/hash equals the evidence checkpoint. Historical unacknowledged rows must drain; freshly committed concurrent events may await the next export within the configured lag limit. Run the full chain verifier and confirm a subsequent export preserves those guarantees before restoring the timer's previous active state. The segment's `source_commit` identifies the running application image, not the host exporter. Caddy rotations use the default millisecond timestamp with optional `-size`/`-time` reason and `.gz`; legacy names without a reason remain supported. Unknown formats, duplicate timestamps, and unprovable cursor continuity fail closed.
