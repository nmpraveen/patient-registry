...
Admin > User Management > Roles exposes **Manage PhoneBook contacts** independently of settings access. New Admin and Reception defaults enable it. Migration patients.0045 enables the existing Reception role and all existing settings-manager roles, preserving every other customized flag. Normal default-role initialization does not overwrite a later revocation. Newly created custom roles need an explicit grant. GET /api/me/ exposes capabilities.manage_phonebook; contact write/inactive-read authorization uses that capability only. Reception does not gain settings, user/device administration, announcement publication, unrestricted reminders, backups or extra clinical scope. Existing fresh locked write, session/JWT revocation, audit rollback and version-conflict checks remain unchanged.
...
Browser no-store/CSP and existing session/device security remain in force. Native staff entry and polling require a verified capabilities.staff_operations=true profile; absent capability defaults to disabled. Staff 403 or a denied profile clears staff state and ends polling. Authorization generations reject delayed denials from earlier allowed profiles, and a newer verified allowed profile can re-enable access. Staff and clinical foreground requests share the account API client and token-refresh lock. Native consumption is online and keeps only session-owned in-memory state, clears it on account change/revocation or failed refresh, refreshes on entry/resume/mutation, and enforces announcement expiry against server_now and monotonic elapsed time. No new notification provider, push delivery, Room migration or outbox format is implied by these apps.
