...
Web and mobile task writes recheck current authorization and scope, then serialize case/task updates. Existing future ANC completion and completed-to-Scheduled reopening permissions remain enforced. Cancelled tasks retain that status in both full editors; their notes and descriptive fields remain editable. Web and API full completion share RCH follow-up creation under the same transaction and idempotent write boundary. Task changes and their clinical activity commit together. Full edits record changed before/after values, actor and timestamp; security audit records retain their existing limited metadata.
...
Case detail and call-write responses share call serialization. `reason`, `task_title` and `staff_user` are additive strings; unknown historical values remain empty. `created_at` is immutable server receipt time; `client_event_at` is the optional captured attempt time. Native case call history and patient recent calls are bounded to the newest 20 ordered by receipt time then id descending. Native timestamp parsing retains microseconds on API 24. Ordinary list refreshes preserve independent call history, including when active filters exclude the case; explicit cursor/dataset reset, account invalidation and detail access-denial purges remain enforced. This is not the complete history; the Stage 4 combined timeline work remains separate.

## Migration, bundles and deployment boundary

Migration `0041_calllog_reason` adds a field with empty Python and persistent database defaults; it does not invent historical reasons. Old ORM inserts that omit the column continue to work if the additive schema is retained during a code rollback. Patient bundle version 3 carries reasons; current imports explicitly accept versions 1, 2 and 3, with missing old reasons restored as empty. Version 2 patient-reference validation remains enforced. Earlier readers reject version 3 instead of silently dropping its reasons. Task references, actors, timestamps and dataset invalidation remain intact. Synthetic seed calls include reasons and both task-associated and taskless examples. Full database backups include the new field.
