...
Recent Cases diagnosis/notes edits lock and update only the Case through the mandatory audit helper, avoiding the Patient lock hidden in Case.save. Activity uses the freshly locked before-values; clinical state and identity fields are untouched. A changed or merged Patient link detected at save time rejects without writing. Concurrent patient identity edits and Recent Cases writes complete without losing either change, and audit failure rolls the update back.

After an ANC action's authoritative detail refresh succeeds, native refreshes the active list and counters with its existing bucket, search, assignment scope and category filters. The request snapshot is cleared on account reset and uses the existing account-guarded refresh path. A list refresh failure reports a refresh problem separately from the saved action; it does not turn success into a mutation retry. Coroutine cancellation propagates through both ANC error paths without invoking a failure or success report from the cancelled operation.

Full web case edits submit a signed baseline from the rendered page, bound to the case, editor, case timestamp and linked Patient ID/timestamp. POST locks Patient before Case to match patient identity editing, then reloads the scoped Case and rechecks linkage before validating the original draft. A later clinical or identity update makes the original page stale; rejection preserves the draft and its original token for review/reload. Missing/invalid baselines also reject without saving. Full identity edits retain their existing mirrors and mandatory audit. API patch requests retain their separate existing baseline contract. ANC action visibility uses the normalized domain workflow, including mixed-case department names.
