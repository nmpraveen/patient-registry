...
Room schema 11 makes every cache, push-token, conflict, and pending-write key account-qualified. Schema 12 adds a persistent account generation/revocation tombstone. Every repository, paging, worker, cache, outbox, conflict, and push-token write verifies that generation inside the same Room transaction, so an old operation cannot recreate owner rows after logout or account switch. Background work carries an account-qualified unique identity and re-verifies `/me` before using a refreshed JWT; a worker stops without deleting or replaying a row if the active account changes.

The schema 10-to-11 ownership transition intentionally discards all unowned local rows. Earlier rows cannot be assigned safely to the next login, so cached server data is downloaded again only after login or restore has atomically completed refresh, `/me`, and stable account-ID verification. The 11-to-12 migration preserves already-owned rows but leaves them inactive until the stored account is verified again. The previous plaintext `medtrack.db` and sidecars are deleted; new local PHI is stored in the SQLCipher-backed `medtrack_secure.db` using a random passphrase held in Android Keystore-backed encrypted preferences.

Retryable refresh or `/me` failures (transport, timeout, and server errors) keep the verified owner binding, encrypted rows, refresh credential, outbox, and lock while the UI remains behind the secure retry gate; they never commit a candidate identity or display another account's cache. A 401/403, invalid credential, malformed identity, or identity mismatch routes app, worker, and push through one compare-and-invalidate operation: await account-work cancellation, deactivate live visibility, atomically revoke and purge owner rows, clear the owner lock, then clear credentials only if the captured account session incarnation is still current. Automatic HTTP refresh uses that same immutable incarnation plus the expected-account binding and retries the original clinical request only after isolated `/me` verification and a successful Boolean session commit.
...
The account boundary necessarily touches `MedtrackSyncWorker`, repository queue/drain code, push-token registration, automatic network refresh, and app-container sync startup. Keep those files as explicit merge-review points for concurrent sync/release work. The server mobile-device approval seam remains intentionally unchanged until the backend publishes its final request contract; no approval fields were guessed, and no server endpoint contract, FCM payload shape, build flavor, Docker, or operations behavior changes here.
